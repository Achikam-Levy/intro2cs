...
def test_car_init():
    car = Car('Y', 3, (1, 0), 1)
    assert car.get_name() == 'Y'
    assert car.length == 3
    assert car.location == (1, 0)
    assert car.orientation == 1

    # edge cases
    assert (Car('O', 1, (1, 0), 1)).length == 1
    assert (Car('foo', 2, (0, 0), 0)).get_name() == 'foo'
    assert (Car('A', 5, (-1, -1), 0)).location == (-1, -1)

    # illegal values
    assert (Car('R', 0, (0, 0), 0)).length == 0
    assert (Car('W', 1, (0, 0), -1)).location != -1
...
def test_coordinates():
    ...
    assert Car('Y', 10, (0, -2), 1).car_coordinates() == [(0, i) for i in range(-2, 8)]
...
def test_movement_requirements():
    ...
    # illegal
    # assert len(Car('Y', 2, (1, 1), 0).movement_requirements('r')) < 1
    # assert len(Car('Y', 2, (1, 1), 0).movement_requirements('l')) < 1
    # assert len(Car('Y', 2, (1, 1), 1).movement_requirements('d')) < 1
    # assert len(Car('Y', 2, (1, 1), 1).movement_requirements('u')) < 1
